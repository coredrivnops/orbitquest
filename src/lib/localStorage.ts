...
const DEFAULT_PROGRESS: PlayerProgress = {
    stardust: 0,
    unlockedPlanets: ['earth', 'mars'], // Earth and Mars are free
    highScores: {},
    lastPlayed: null,
};
...
export function loadProgress(): PlayerProgress {
    ...
    try {
        ...
        if (saved) {
            ...
            // Ensure earth and mars are always unlocked
            if (!parsed.unlockedPlanets.includes('earth')) {
                parsed.unlockedPlanets.push('earth');
            }
            if (!parsed.unlockedPlanets.includes('mars')) {
                parsed.unlockedPlanets.push('mars');
            }
            return parsed;
        }
    } catch (error) {
